% Code to automatically segment Nup153 and Tpr particles, calculate their
% centroids, measure distance between centroids, measure integrated
% intensities...
...
%TprImage = imread('200707_sc_1_Tpr.tif');
originalImage = imread('200707_sc_2_Nup153.tif');

filteredImage = imgaussfilt(originalImage);
...
binaryImage = filteredImage > thresholdValue; % Bright objects will be chosen if you use >.
...
for k = 1 : numberOfBlobs           % Loop through all blobs.
    ...
    blobCircularity = blobMeasurements(k).Circularity;
...
allowableAreaIndexes = (allBlobAreas > 9) & (allBlobAreas<100); % Take the small objects.
...
keeperCentroids = ismember(allBlobCentroids, keeperIndexes);
keeperCentroidsX = keeperCentroids(1:2:end-1);
keeperCentroidsY = keeperCentroids(2:2:end);
...
imshow(originalImage);
axis image;
title('"Keeper"', 'FontSize', captionFontSize);
hold on 
plot(keeperCentroidsX, keeperCentroidsY, '.');
...
for k = 1 : numberOfBlobs           % Loop through all keeper blobs.
	...
	if itsADime
		...
		plot(centroidsX(k), centroidsY(k), 'bx');
	end
end
%}
